...
export const ClassificationInput = forwardRef<
  HTMLDivElement,
...
>(({ onClassify, isLoading }, ref) => {
  ...
  const [extractedData, setExtractedData] =
    useState<ExtractedPharmaData | null>(null);
  ...
  const handleFileSelect = async (file: File | null, type: "spec" | "msds") => {
    ...
    if (file) {
      ...
      try {
        ...
        setExtractedData((prev) => {
          if (prev) {
            return {
              ...
              casNumbers: [
                ...new Set([...prev.casNumbers, ...data.casNumbers]),
              ],
              activeIngredients: [
                ...new Set([
              ...
              safetyWarnings: [
                ...new Set([...prev.safetyWarnings, ...data.safetyWarnings]),
              ],
              chemicalComposition: [
                ...new Set([
              ...
              formulation: [
                ...new Set([...prev.formulation, ...data.formulation]),
              ],
              packaging: [...new Set([...prev.packaging, ...data.packaging])],
              therapeuticUse: [
                ...new Set([...prev.therapeuticUse, ...data.therapeuticUse]),
              ],
        ...
        toast.error(
          `Could not extract text from PDF: ${errorMessage}. Try pasting the content manually.`,
          { duration: 5000 },
        );
        // Reset the file since extraction failed
        if (type === "spec") {
          setSpecSheet(null);
        } else {
          setMsds(null);
  ...
  const handleClassify = () => {
    // Prioritize extracted data from PDFs, fallback to manual text input
    let combinedText = "";
  ...
  const canClassify =
    (extractedData?.rawText || text.trim()) && !isLoading && !isExtracting;
            ...
            <CardContent className="p-6 md:p-8 space-y-6">
              ...
              <Tabs defaultValue="spec" className="w-full">
                <TabsList className="grid w-full grid-cols-2 mb-4 h-12 rounded-xl">
                  ...
                  <TabsTrigger
                    value="msds"
                    className="flex items-center gap-2 rounded-lg data-[state=active]:bg-warning data-[state=active]:text-warning-foreground"
                  >
                ...
                <TabsContent value="msds" className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Include CAS numbers, hazard classifications, and safety
                    warnings
                  </p>
                  <FileUploader
              ...
              <div className="space-y-3">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Include: Active ingredients, CAS numbers, formulation,
                  packaging, therapeutic use
                </p>
              </div>
